...
void do_comment( CHAR_DATA *ch, char *argument )
{
	...
	case SUB_WRITING_NOTE:
	  ...
	  if ( ch->dest_buf != ch->pcdata->pnote )
	     bug( "do_comment: sub_writing_note: ch->dest_buf != ch->pcdata->pnote", 0 );
...
void do_diary( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    FILE *fp;
    char * strtime;
	...
	case SUB_WRITING_NOTE:
	  if ( !ch->pcdata->pnote )
	  {
	     bug( "do_diary: note got lost?", 0 );
	     send_to_char( "Your note got lost!\n\r", ch );
	     stop_editing(ch);
	     return;
	  }
	  if ( ch->dest_buf != ch->pcdata->pnote )
	     bug( "do_comment: sub_writing_note: ch->dest_buf !=
ch->pcdata->pnote", 0 );
	  STRFREE( ch->pcdata->pnote->text );
	  ch->pcdata->pnote->text = copy_buffer( ch );
	  stop_editing( ch );	   
	...
	strtime[strlen(strtime)-1]	= '\0';
	ch->pcdata->pnote->date			= STRALLOC( strtime );
        ch->pcdata->pnote->sender		= QUICKLINK(ch->name);
        return;
    }
 
/*	pnote->sender   = ch->pcdata->pnote->sender;
        pnote->date     = ch->pcdata->pnote->date;
	pnote->text	= ch->pcdata->pnote->text;*/

    if ( !str_cmp( arg, "purge" ) )
    {
     if ( ch->level < 58 )
     {
     send_to_char("Only a COUNCIL or higher may purge the diary.\n\r",ch);
     return;
     }

     remove(DIARY_FILE);
     send_to_char("Diary File Purged.\n\r",ch);
     return;
    }
	...
	if ( !ch->pcdata->pnote )
        {
          send_to_char("Write an Entry First!\n\r",ch);
	  return;
	}

	if ( ( fp = fopen( DIARY_FILE, "a" ) ) == NULL )
	{
            bug("Cannot Open Diary File!",0);
	    perror( DIARY_FILE );
	}
	else
	{
	    fprintf( fp, "[%s] %-10s:\n%s",
		ch->pcdata->pnote->date,
		ch->pcdata->pnote->sender,
		ch->pcdata->pnote->text
		);
	    new_fclose( fp );
	}
	send_to_char( "Diary Entry Written.\n\r", ch );
	if ( ch->pcdata->pnote )
	{
	    STRFREE( ch->pcdata->pnote->text );
	    STRFREE( ch->pcdata->pnote->subject );
    ...
    send_to_char( "Eh?  Type 'help diary' for usage (*mutter*).\n\r", ch );
    return;
}
