...
{
  for (po=in_zone->first_obj; po; po=po->next)   /* Loop through allobjects */
  {
...
OBJ_INDEX_DATA *pObj;
OBJ_INDEX_DATA **freq=NULL;                        /* dynamic array of pointers */
...
for( in_zone = first_zone; in_zone; in_zone = in_zone->next )
{
   ...
   {
      if ( in_zone->room_index_hash[cou] )
         for (pRoom = in_zone->room_index_hash[cou]; pRoom; pRoom =pRoom->next)
         {
...
for( in_zone = first_zone; in_zone; in_zone = in_zone->next )
{
   for (cou = 0; cou < MAX_KEY_HASH; cou++) {     /* loop thru obj_index_hash */
       if ( in_zone->obj_index_hash[cou] )                 /* safety check*/
          for (pObj=in_zone->obj_index_hash[cou];          /* loop thruall pObjInd */
               pObj; pObj=pObj->next)
...
{
   for (cou = 0; cou < MAX_KEY_HASH; cou++)     /* loop thru obj_index_hash */
       if ( in_zone->obj_index_hash[cou] )
          for (pObj=in_zone->obj_index_hash[cou]; pObj; pObj=pObj->next)
              if (pObj->weight == 0) {
                 zero_obj_ind++;
                 zero_obj += pObj->count;
...
for( in_zone = first_zone; in_zone; in_zone = in_zone->next )
{
for (po=in_zone->first_obj; po; po=po->next)
{
...
for( in_zone = first_zone; in_zone; in_zone = in_zone->next )
{
   for (cou = 0; cou < MAX_KEY_HASH; cou++)
   {
      if ( in_zone->mob_index_hash[cou] )
         for (pm = in_zone->mob_index_hash[cou]; pm; pm = pm->next)
         {
