...
void clean_area	    args( ( AREA_DATA *pArea ) );

bool victim_home_zone	args( ( AREA_DATA *pArea, CHAR_DATA *victim ) );
bool object_home_zone	args( ( AREA_DATA *pArea, OBJ_DATA *obj ) );
                    ...
                    AREA_DATA *pArea )
{
  ...
  int pr;
  int zone;
  record_call("<is_room_reset>");

  if ( !aRoom )
    return TRUE;

   if ( pArea && pArea->zone )
  zone = (int)pArea->zone->number;
   else
  zone = 1;
  ...
  case 'M':
  case 'O':
    pRoom = get_room_index( pReset->arg3,zone );
  ...
  case 'H':
    ...
    for ( reset = pReset->prev; reset; reset = reset->prev )
      if ( (reset->command == 'O' || reset->command == 'P' ||
           ...
           (!pr || pr == reset->arg1) && get_obj_index(reset->arg1,zone) )
        break;
  ...
  case 'B':
    ...
    case BIT_RESET_MOBILE:
      for ( reset = pReset->prev; reset; reset = reset->prev )
        if ( reset->command == 'M' && get_mob_index(reset->arg1,zone) )
          break;
      if ( reset && is_room_reset(reset, aRoom, pArea) )
        return TRUE;
      return FALSE;
  ...
  case 'D':
  case 'R':
    pRoom = get_room_index( pReset->arg1,zone );
                            ...
                            ROOM_INDEX_DATA *pRoom )
{
  ...
  if ( arg[0] == '\0' )
    ...
    pRoom = get_room_index(atoi(arg),zone);
  if ( !pRoom )
  {
			...
			ROOM_INDEX_DATA *pRoom, char *name)
{
  ...
  else
  {
    ...
    for ( reset = pArea->first_reset; reset; reset = reset->next )
    {
      ...
      if ( (pObjTo = get_obj_index(reset->arg1,zone)) &&
			...
			ROOM_INDEX_DATA *pRoom, char *name)
{
  ...
  else
  {
    ...
    for ( reset = pArea->first_reset; reset; reset = reset->next )
    {
      ...
      if ( (pMob = get_mob_index(reset->arg1,zone)) &&
		 ...
		 ROOM_INDEX_DATA *aRoom )
{
  ...
  int	zone;
  AREA_DATA *temp, *temp_next;
  
  argument = one_argument(argument, arg);
  record_call("<edit_reset>");
  ...
  if ( !str_cmp(arg, "all" ) )
  {
   ...
   send_to_char("Starting Areas:\n\r",ch);
   for ( temp = first_area; temp; temp = temp_next )
   {
     temp_next = temp->next;
     ch_printf( ch, "%s..", temp->filename );
     num = temp->nplayer;
     temp->nplayer = 0;
     SET_BIT( temp->flags, AFLAG_STARTED );
     reset_area(temp);
     temp->nplayer = num;
     send_to_char("..Done.\n\r",ch);
   }     
   send_to_char("All areas Started.\n\r",ch);
   return;
   }
  ...
  if ( !str_prefix( arg, "mobile" ) )
  {
    ...
    if ( !(pMob = get_mob_index(atoi(arg),zone)) )
    {
  ...
  if ( !str_prefix(arg, "object") )
  {
    ...
    if ( !(pObj = get_obj_index(atoi(arg),zone)) )
    {
...
/*    pReset = make_reset('H', 1, reset->arg1, 0, 0);*/
  ...
  if ( ch->substate == SUB_REPEATCMD )
  {
    ...
    ch->substate = SUB_REPEATCMD;
    ch->pcdata->last_cmd = (aRoom ? do_rreset : do_reset);
  }
  else
...
void do_instaroom( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !can_rmodify(ch, pRoom) )
    return;
  if ( pRoom->area != pArea && get_trust(ch) < LEVEL_GOD )
  {
...
void do_instazone( CHAR_DATA *ch, char *argument )
{
  ...
  {
    if ( !(pRoom = get_room_index(vnum,zone)) || pRoom->area != pArea )
      continue;
    instaroom( pArea, pRoom, dodoors );
  }
...
int generate_itemlevel( AREA_DATA *pArea, OBJ_INDEX_DATA *pObjIndex )
{
    ...
    record_call("<generate_itemlevel>");
...
bool victim_home_zone( AREA_DATA *pArea, CHAR_DATA *victim )
{
MOB_INDEX_DATA *search;
int cnt=0;
int zone;
record_call("<victim_home_zone>");
...
for( cnt = pArea->low_m_vnum; cnt <= pArea->hi_m_vnum; cnt++ )
{

if( (search=get_mob_index( cnt, zone ) )==NULL )
continue;

if (victim->pIndexData == search)
return TRUE;
...
bool object_home_zone( AREA_DATA *pArea, OBJ_DATA *obj )
{
OBJ_INDEX_DATA *search;
int cnt=0;
int zone;
record_call("<object_home_zone>");
...
for( cnt = pArea->low_o_vnum; cnt <= pArea->hi_o_vnum; cnt++ )
{

if( (search=get_obj_index( cnt, zone ) )==NULL )
continue;

if (obj->pIndexData == search)
return TRUE;
...
void clean_area( AREA_DATA *pArea )
{
int cnt = 0;
int zone;
record_call("<clean_area>");
...
fprintf(stderr,"Cleaning Area: %s.\n",pArea->name);
for ( cnt = pArea->low_r_vnum; cnt <= pArea->hi_r_vnum; cnt++ )
{
...
extern bool fBootDb;
   ...
   if ( fBootDb == TRUE )
   {
	...
	if( (room=get_room_index( cnt, zone ) )==NULL )
	 continue;
   ...
   if ( room->first_person )
   {
        for ( victim = room->first_person; victim; victim = vnext )
        {
		 ...
		 && (victim_home_zone(pArea,victim) == TRUE) )
    ...
    if ( room->first_content )
    {
	  ...
	  if( object_home_zone(pArea,obj) == TRUE )
            extract_obj( obj );
        }
    }
}
...
/* makes weird shit happen .. shrug --GW*/
/*  clean_area(pArea);*/
  ...
  for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
  {
    ...
    {
    default:
      bug( "Reset_area: bad command %c. (%s)", pReset->command, pArea->filename );
      break;
    
    case 'M':
      if ( !(pMobIndex = get_mob_index(pReset->arg1,zone)) )
      {
        bug( "Reset_area: 'M': bad mob vnum %d. (%s)", pReset->arg1,pArea->filename );
        continue;
      }
      if ( !(pRoomIndex = get_room_index(pReset->arg3,zone)) )
      {
        bug( "Reset_area: 'M': bad room vnum %d. (%s)", pReset->arg3,
pArea->filename );
        continue;
      }
      ...
      {
        ROOM_INDEX_DATA *pRoomPrev = get_room_index(pReset->arg3 - 1,zone);
    ...
    case 'G':
    case 'E':
      if ( !(pObjIndex = get_obj_index(pReset->arg1,zone)) )
      {
        bug( "Reset_area: 'E' or 'G': bad obj vnum %d. (%s)", pReset->arg1,
pArea->filename );
        continue;
      }
      if ( !mob )
	...
	int olevel = generate_itemlevel( pArea, pObjIndex );
        obj = create_object(pObjIndex, olevel,in_zone);
	if ( !obj )
	break;
        SET_BIT(obj->extra_flags, ITEM_INVENTORY);
      }
      else
        obj = create_object(pObjIndex, number_fuzzy(level),in_zone);
    ...
    case 'O':
      if ( !(pObjIndex = get_obj_index(pReset->arg1,zone)) )
      {
        bug( "Reset_area: 'O': bad obj vnum %d. (%s)", pReset->arg1,pArea->filename );
        continue;
      }
      if ( !(pRoomIndex = get_room_index(pReset->arg3,zone)) )
      {
        bug( "Reset_area: 'O': bad room vnum %d. (%s)", pReset->arg3,pArea->filename );
        continue;
      }
      if ( pArea->nplayer > 0 ||
      ...
      obj = create_object(pObjIndex,number_fuzzy(generate_itemlevel(pArea, pObjIndex)),in_zone);
	if( !obj )
		break;
      obj->level = UMIN(obj->level, LEVEL_AVATAR);
      obj->cost = 0;

      /* Initialize Bonuses --GW */
      obj->dam_bonus      = 0;
      obj->hit_bonus      = 0;
      obj->ac_bonus       = 0;
      obj->mac_bonus      = 0;
      /* upstatting? */
      obj=obj_upstat_check(obj);
    ...
    case 'P':
      if ( !(pObjIndex = get_obj_index(pReset->arg1,zone)) )
      {
        bug( "Reset_area: 'P': bad obj vnum %d. (%s)",pReset->arg1,pArea->filename );
        continue;
      }
      if ( pReset->arg3 > 0 )
      {
/*	OBJ_DATA *search;*/
	...
	cnt = 0;
        ...
        if ( !(pObjToIndex = get_obj_index(pReset->arg3,zone)) )
        {
          bug( "Reset_area: 'P': bad objto vnum %d. (%s)", pReset->arg3,pArea->filename );
          continue;
        }
        if ( pArea->nplayer > 0 ||
...
/* wrong awnser --GW */
/*	for ( search = to_obj->first_content; search; search =search->next)
	{
	   if ( search == obj )
	   cnt++;
        }
	 ...
	 obj = NULL;
      ...
      else
      {
        ...
        for ( iNest = 0; iNest < pReset->extra; iNest++ )
          ...
          {
            bug( "Reset_area: 'P': Invalid nesting obj %d. (%s)", pReset->arg1,pArea->filename );
            iNest = -1;
            break;
          }
        if ( iNest < 0 )
      ...
      obj = create_object(pObjIndex,number_fuzzy(UMAX(generate_itemlevel(pArea,pObjIndex),to_obj->level)),in_zone);
      if (!obj)
	break;
      obj->level = UMIN(obj->level, LEVEL_AVATAR); 
    ...
    case 'T':
      if ( IS_SET(pReset->extra, TRAP_OBJ) )
      {
        ...
        if ( pReset->arg3 > 0 )
        {
          if ( !(pObjToIndex = get_obj_index(pReset->arg3,zone)) )
          {
            bug( "Reset_area: 'T': bad objto vnum %d. (%s)", pReset->arg3,
pArea->filename );
            continue;
          }
          if ( pArea->nplayer > 0 ||
      ...
      else
      {
        if ( !(pRoomIndex = get_room_index(pReset->arg3,zone)) )
        {
          bug( "Reset_area: 'T': bad room %d. (%s)", pReset->arg3, pArea->filename );
          continue;
        }
        if ( pArea->nplayer > 0 ||
             count_obj_list(get_obj_index(OBJ_VNUM_TRAP,1),
    ...
    case 'H':
      if ( pReset->arg1 > 0 )
      {
        if ( !(pObjToIndex = get_obj_index(pReset->arg1,zone)) )
        {
          bug( "Reset_area: 'H': bad objto vnum %d. (%s)", pReset->arg1,
pArea->filename );
          continue;
        }
        if ( pArea->nplayer > 0 ||
    ...
    case 'B':
      ...
      case BIT_RESET_DOOR:
        ...
        if ( !(pRoomIndex = get_room_index(pReset->arg1,zone)) )
        {
          bug( "Reset_area: 'B': door: bad room vnum %d. (%s)", pReset->arg1,
pArea->filename );
          continue;
        }
      ...
      case BIT_RESET_ROOM:
        if ( !(pRoomIndex = get_room_index(pReset->arg1,zone)) )
        {
          bug( "Reset_area: 'B': room: bad room vnum %d. (%s)", pReset->arg1,
pArea->filename );
          continue;
        }
      ...
      case BIT_RESET_OBJECT:
        if ( pReset->arg1 > 0 )
        {
          if ( !(pObjToIndex = get_obj_index(pReset->arg1,zone)) )
          {
         bug( "Reset_area: 'B': object: bad objto vnum %d. (%s)", pReset->arg1,
pArea->filename );
            continue;
          }
	...
	ext_bv = TRUE;
        break;
      default:
        bug( "Reset_area: 'B': bad options %d. (%s)", pReset->arg2,
pArea->filename );
        continue;
      }
    ...
    case 'D':
      if ( !(pRoomIndex = get_room_index(pReset->arg1,zone)) )
      {
        bug( "Reset_area: 'D': bad room vnum %d. (%s)", pReset->arg1,
pArea->filename );
        continue;
      }
    ...
    case 'R':
      if ( !(pRoomIndex = get_room_index(pReset->arg1,zone)) )
      {
        bug( "Reset_area: 'R': bad room vnum %d. (%s)", pReset->arg1,
pArea->filename );
        continue;
      }
		  ...
		  int start, int end )
{
  ...
  for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
  {
    ...
    default:
      ...
      break;
    case 'M':
      if ( !(mob = get_mob_index(pReset->arg1,zone)) )
        mname = "Mobile: *BAD VNUM*";
      else
        mname = mob->player_name;
      if ( !(room = get_room_index(pReset->arg3,zone)) )
        rname = "Room: *BAD VNUM*";
      else
        rname = room->name;
      ...
      if ( !room )
        mob = NULL;
      if ( (room = get_room_index(pReset->arg3-1,zone)) &&
    ...
    case 'E':
      ...
      if ( !(obj = get_obj_index(pReset->arg1,zone)) )
    ...
    case 'P':
      ...
      if ( pReset->arg3 > 0 )
      {
        obj2 = get_obj_index(pReset->arg3,zone);
      ...
      else
      {
        ...
        else if ( !(obj2 = get_obj_index(reset->arg1,zone)) )
    ...
    case 'H':
      if ( pReset->arg1 > 0 )
        if ( !(obj2 = get_obj_index(pReset->arg1,zone)) )
    ...
    case 'B':
      ...
      case BIT_RESET_DOOR:
        ...
        if ( !(room = get_room_index(pReset->arg1,zone)) )
          rname = "Room: *BAD VNUM*";
        else
          rname = room->name;
      ...
      case BIT_RESET_MOBILE:
        if ( pReset->arg1 > 0 )
        {
          ...
          if ( !(mob2 = get_mob_index(pReset->arg1,zone)) )
...
void renumber_put_resets( AREA_DATA *pArea )
{
  ...
  record_call("<renumber_put_resets>");
