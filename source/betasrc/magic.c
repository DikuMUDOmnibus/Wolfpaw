 ...
 * M####	check if player has #### amount of movepoints - Cal
 *
 * Special operators:
	...
	switch( UPPER(arg[0]) )
	{
		...
		if( ch->move >= value )
		{
		  ...
		  else
		  {
		     if( consume )
		     {
			send_to_char( "&gYou feel a little weaker...\n\r",ch );
			ch->move -= value;
		     }
		     continue;
		  }
...
return;
	...
	/* And Why not! --GW */
	if ( IS_NPC(ch) && IS_AFFECTED( ch, AFF_CHARM ) && !IS_SET(ch->act,ACT_CREATE_LIFE_MOB))
	{
	...
	{
	    /* NPC can cast anything the creator can --GW */
	    if (  IS_NPC(ch) && IS_SET(ch->act, ACT_CREATE_LIFE_MOB ) 
		  && ch->master )
	    {
	    if ( ( sn = find_spell( ch->master, arg1, TRUE ) ) < 0
	    || ( !IS_NPC(ch->master) &&
	         !CAN_USE_SK(ch->master, skill_table[sn]) && 
	         !CAN_USE_RACE_SK(ch->master, skill_table[sn]) ) )
	    {
	...
	|| skill->guild != ch->pcdata->clan->class) )
	{
	   ...
	   ( IS_SET( ch->in_room->room_flags, ROOM_SAFE ) ) )
           {
	    send_to_char("You feel to good to cast that here!\n\r",ch);
	    return;
	   }
  
     /* Need the FULL name of spells, or it crashes ....  GW 5/24/97*/

	  if (!IS_NPC(ch) && ( CAN_USE_SK( ch,skill_table[gsn_ancient_lore] ) ||
		CAN_USE_RACE_SK(ch, skill_table[gsn_ancient_lore]) )
 	  &&  ch->pcdata->learned[gsn_ancient_lore] 
          && (number_percent( ) + skill->difficulty * 5 ) < ch->pcdata->learned[sn])
          {
	  act(AT_MAGIC,"$n's knowledge of ancient lore allows $m to cast with startling efficiency.",ch,NULL,NULL,TO_ROOM);
	  act(AT_MAGIC,"Your knowledge of ancient lore allows you to cast with startling efficiency.",ch,NULL,NULL,TO_CHAR);
          mana = UMAX(skill->min_mana, 50 / ( ch->level - SP_MANA(ch, skill) ) );
          learn_from_success( ch, gsn_ancient_lore );
          }
        ...
        if (ch->race != 31)
	{
	 ...
	 &&   !ch->pcdata->learned[gsn_ancient_lore] )
	 {
        mana = (IS_NPC(ch) && ch->race != 31) ? 0 : UMAX(skill->min_mana,
           100 / ( 2 + ch->level - SP_MANA(ch, skill) ) );
	 }
	 if ( !IS_NPC(ch)
	 &&   ch->pcdata->learned[gsn_ancient_lore] )
	   {
        mana = (IS_NPC(ch) && ch->race != 31) ? 0 : UMAX(skill->min_mana,
           100 / ( 2 + ch->level - SP_MANA(ch, skill) ) );
	   learn_from_failure(ch, gsn_ancient_lore);
	   }
	}
...
/*
 * This is antiquated and doesn't work well with AoA :P -- Cal
 *
...
*/
    ...
    ||    ( str_cmp( skill->name, "create sound" ) ) ) /* Psi ventriloquate */
	say_spell( ch, sn );

	if ( IS_SET( ch->in_room->room_flags, ROOM_NO_MAGIC ) )
	{
	    set_char_color( AT_MAGIC, ch );
	    act(AT_RED,"$n winces in pain, Was That Thunder you Heard?",ch,NULL,NULL,TO_ROOM);
	    send_to_char( "Your Head Pounds, thats going to hurt in the morning!\n\r", ch );
	    return;
	}
	...
	if (ch->level < LEVEL_IMMORTAL)    /* so imms dont lose mana */
	  ch->mana -= mana / 2;        /* Well guess what imms do lose mana */
	learn_from_failure( ch, sn );  /* Anyone wanna fix that :P */
	return;
    }
	...
	/* reflect magic stuff .. GW */
        else if (victim &&
	         ( victim->fighting || ch->fighting ) &&
	 	 IS_AFFECTED(victim, AFF_REFLECT_MAGIC) &&
		 (!IS_NPC(victim) || (IS_NPC(victim) && victim != ch)))
        {
        act( AT_MAGIC, "$n glows shortly as the magic reflects.",victim,NULL,NULL,TO_NOTVICT);
        set_char_color( AT_MAGIC, ch );
	send_to_char("\nYou glow shortly as the magic reflects off your shield\n\r",victim);
	return;
        }
	else
	{
...
ch_ret spell_ressurection( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    SKILLTYPE *skill = get_skilltype(sn);
    OBJ_DATA *pin;
    ...
    if ( victim->perm_con <= 0 )
    {
     send_to_char("That body cannot sustain any more ressurections!\n\r",ch);
     return rSPELL_FAILED;
    }
    
    if ( victim->pcdata->old_hp == 0 || victim->pcdata->clssdropped == 0 )
    {
     send_to_char( "You cannot Ressurect that person!\n\r",ch);
     failed_casting( skill, ch, victim, NULL );
     return rSPELL_FAILED;
    }
    ...
    if ( victim == ch )
    {
     send_to_char( "This spell can only be cast on another.\n\r",ch);
     failed_casting( skill, ch, victim, NULL );
     return rSPELL_FAILED;
    }

    /* Ok! Start the Res! -- Nail the caster --GW */
    act( AT_MAGIC,"$n begins to chant, begging the gods for ressurection",ch,NULL,NULL,TO_ROOM);
    act( AT_RED,"A Large swirling portal appears above $n!",victim,NULL,NULL,TO_ROOM);
    act( AT_BLOOD,"A Wave of pain rips through your mind!",ch,NULL,NULL,TO_CHAR);
    ch->mana = 0;
    ch->hit = 5;
    ch->move = 50;
    save_char_obj(ch);

    /* Fix up the person casted on --GW */
    de_equip_char(victim);
    victim->max_hit = victim->pcdata->old_hp;    
    victim->max_mana = victim->pcdata->old_mana;    
    victim->max_move = victim->pcdata->old_move;    
    victim->exp = victim->pcdata->old_exp;    
    save_char_obj(victim);

    if ( !IS_SET(victim->pcdata->flagstwo, MOREPC_KNIGHT) )
    {
       victim->perm_con--;
       victim->perm_str--;
       victim->practice -= 2;
    ...
    {
	act(AT_RED,"$N holds $S head high! $S's Crest of Knighting Glows..",ch,NULL,victim,TO_ROOM);
	act(AT_RED,"You hold your head high! Your Crest of Knighting Glows..",ch,NULL,victim,TO_VICT);
    }

    victim->pcdata->old_hp = 0;
    victim->pcdata->old_mana = 0;
    victim->pcdata->old_move = 0;
    victim->pcdata->old_exp = 0;
    /* Oops .. glory pins should not come off! --GW */
    pin=get_pin(victim);
    if ( pin )
    equip_char( victim, pin, WEAR_INSIG );

/*
 * Changed this to be the Loss reveresed, to avoid giving levels
 * where they weren't taken -GW
 */

switch( victim->pcdata->clssdropped )
{
case 1:
  victim->level = URANGE(0,victim->level+1,50);
  break;
case 2:
  victim->level2 = URANGE(0,victim->level2+1,50);
  break;
case 3:
  victim->advlevel = URANGE(0,victim->advlevel+1,50);
  break;
case 4:
  victim->advlevel2 = URANGE(0,victim->advlevel2+1,50);;
  break;
}
    /* Smash to 51 bug --GW */
    victim->pcdata->clssdropped = 0;

    save_char_obj(victim);
    act( AT_MAGIC,"$n glows shortly and looks much better!",victim,NULL,NULL,TO_ROOM);
    send_to_char("You Constitution and Strength drop by 1..\n\r",victim);
    send_to_char("You have been Ressurected!\n\r",victim);
    return rNONE;
}

ch_ret spell_replenish_blood( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    if( !IS_VAMPIRE(ch) )
       {
     send_to_char( "&RYou have no use for this spell\n\r", ch );
     return rSPELL_FAILED;
       }

    if( ch->pcdata->condition[COND_BLOODTHIRST] == get_bloodthirst(ch) )
       {
     send_to_char( "&RYour veins are as full as they can be\n\r", ch );
     return rSPELL_FAILED;
       }
    if( ch->move < 100 )
       {
     send_to_char( "You don't have enough energy to convert\n\r", ch );
     return rSPELL_FAILED;
       }
    af.type	 = sn;
    ...
    /* I have reservations as to weither or not APPLY_BLOOD works so
       for now lets do it this way -- Cal */

    ch->pcdata->condition[COND_BLOODTHIRST] += dice(2,5);
    send_to_char( "&rYour veins are flowing with the power of blood!!\n\r", ch );
...
/*
    to_channel( buf, CHANNEL_MONITOR, "[ ** ", UMAX( LEVEL_IMMORTAL,
ch->level ) );
*/
    return rNONE; 
}
...
ch_ret spell_create_food( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    zone = find_zone(1);
    mushroom = create_object( get_obj_index( OBJ_VNUM_MUSHROOM,1 ),0,zone );
...
ch_ret spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	while ( victim->first_affect )
	   affect_remove( victim, victim->first_affect );
           ...
           if ( race_table[victim->race] && !xIS_EMPTY(race_table[victim->race]->affected))
    ...
    {
    if ( victim && victim->race && !IS_NPC(victim) 
 	&& race_table[victim->race] && !xIS_EMPTY(race_table[victim->race]->affected))
    {
    if ( xHAS_BITS(victim->affected_by, race_table[victim->race]->affected)
    ||   level < victim->level
	...
	if ( IS_AFFECTED(victim, affected_by) )
	    break;
	if ( cnt++ > 30 )
	{
	    failed_casting( skill, ch, victim, NULL );
...
/* Code "borrowed" from spell_enchant_weapon ;) -- Cal */
ch_ret spell_enchant_armor( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
      SET_BIT( obj->extra_flags, ITEM_ANTI_EVIL );
      act( AT_BLUE, "$p shines with a blue aura.", ch, obj, NULL, TO_CHAR );
      act( AT_BLUE, "$p glows blue.", ch, obj, NULL, TO_ROOM );
    }
    if ( IS_EVIL(ch) )
    {
	SET_BIT( obj->extra_flags, ITEM_ANTI_GOOD );
	act( AT_RED, "$p glows ominously with a deep red aura.",ch,obj,NULL,TO_CHAR);    
	act( AT_RED, "$p glows red.", ch, obj, NULL, TO_ROOM );
    }
    if ( IS_NEUTRAL(ch) )
    {
      SET_BIT( obj->extra_flags, ITEM_ANTI_GOOD );
      SET_BIT( obj->extra_flags, ITEM_ANTI_EVIL );
      act( AT_YELLOW, "$p glows yellow.", ch, obj, NULL, TO_CHAR );
      act( AT_YELLOW, "$p glows yellow.", ch, obj, NULL, TO_ROOM );
    }
...
ch_ret spell_energy_drain( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	gain_exp( victim, number_range( level / 2, 3 * level / 2 ), FALSE );
	victim->mana	/= 2;
	victim->move	/= 2;
	dam		 = dice(1, level);
	ch->hit		+= dam;
...
CHAR_DATA *mob;
    ...
    mob = create_mobile(get_mob_index(MOB_VNUM_VAMPIRE,1),find_zone(1));

    if ( !ch )
    {
	bug("Spell_Gate: Null Ch!",0);
        send_to_char("A Bug occured, the MUD was unable to complete your request.\n\r",ch);
	return rNONE;
    }

    if ( !mob )
    {
	bug("Spell_Gate: Null Vampire Mob!",0);
        send_to_char("A Bug occured, the MUD was unable to complete your request.\n\r",ch);
	return rNONE;
    }
...
/* Modified by GW to be more informative */
    OBJ_DATA *obj;
    CHAR_DATA *victim;
    AFFECT_DATA *paf;
    SKILLTYPE *sktmp;
...
ch_ret spell_soul_thief( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim;
    SKILLTYPE *skill = get_skilltype(sn);
    AFFECT_DATA paf;
    ...
    if( IS_GOOD(ch) && IS_GOOD(victim) )
       {
     set_char_color( AT_MAGIC, ch );
     send_to_char( "Their soul matches yours, you can't steal it\n\r", ch );
     return rSPELL_FAILED;
       }
    ...
    /* Begin alignment switch */
     set_char_color( AT_MAGIC, ch );
     set_char_color( AT_MAGIC, victim );
     ch->alignment = victim->alignment;
     ch_printf( ch, "You reach into %s soul and steal %s %s \n\r",
        capitalize(victim->name),
        victim->sex == SEX_MALE ? "his"
      : victim->sex == SEX_FEMALE ? "her" : "its",
        IS_GOOD(victim) ? "morality and gentle nature"
      : IS_EVIL(victim) ? "wickedness and aggression" : "neutrality" );
     ch_printf( victim, "%s examines your soul and starts to mimic it\n\r",
        ch->name );
     send_to_char( "You feel exhausted from the encounter!\n\r", victim );
     
     /* Exertion from having their soul ripped out */
     paf.type     = sn;
     paf.location  = APPLY_MENTALSTATE;
     paf.modifier  = -50;
     paf.duration  = 50;
     xCLEAR_BITS(paf.bitvector);
...
ch_ret spell_sleep( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	to_channel( log_buf, CHANNEL_MONITOR, "[ ** ", UMAX(
LEVEL_IMMORTAL, ch->level ) );
    }
...
ch_ret spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !IS_NPC( ch ) )
    {
	act( AT_MAGIC, "You feel a wave of pain overcome you...", ch,NULL,
	     NULL, TO_CHAR );
	act( AT_MAGIC, "$n winces in pain!", ch, NULL, NULL, TO_ROOM );
	ch->hit -= 40;
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    for ( ; ; )
    {
	pRoomIndex = get_room_index( number_range( 0, 32767 ),1 );
	if ( pRoomIndex )
	...
	&&   !IS_SET(pRoomIndex->area->flags, AFLAG_CLOSED) 
...
ch_ret spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( obj_lose = victim->first_carrying; obj_lose; obj_lose = obj_next )
	{
	    ...
	    case ITEM_CONTAINER:
             /* No Scrap Flag --GW */
             if ( IS_SET( obj_lose->second_flags, ITEM2_NO_SCRAP ) )
             break;
...
ch_ret spell_gas_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( IS_NPC(ch) && IS_AFFECTED( vch, AFF_REFLECT_MAGIC ) )
	{
	  do_say(vch,"Haha! You're puny magic is no match for me!");
	  continue;
	}

	if ( IS_SET( vch->immune, RIS_GAS_BREATH ) )
	{
	  do_say(vch,"Haha! You're puny breath is no match for me!");
	  continue;
	}
...
ch_ret spell_portal( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    */
    ...
    zone = find_zone(fromRoom->area->zone->number);
    portalObj = create_object( get_obj_index( OBJ_VNUM_PORTAL,1 ), 0,zone );
    portalObj->timer = 3;
    ...
    zone = find_zone(targetRoom->area->zone->number);
    portalObj = create_object( get_obj_index( OBJ_VNUM_PORTAL,1 ), 0,zone );
    portalObj->timer = 3;
    STRFREE( portalObj->short_descr );
    portalObj->short_descr = STRALLOC( buf ); 
...
ch_ret spell_recharge( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	||   obj->value[1] > (obj->pIndexData->value[1] * 4) )
	{
	    ...
	    extract_obj(obj);
	    if ( damage(ch, ch, obj->cost, TYPE_UNDEFINED) == rCHAR_DIED
	    ||   char_died(ch) )
		return rCHAR_DIED;
	    else
		return rSPELL_FAILED;
...
ch_ret spell_animate_dead( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( get_mob_index(MOB_VNUM_ANIMATED_CORPSE,1) == NULL )
    {
    ...
    if ( (pMobIndex = get_mob_index((sh_int)
abs(corpse->cost),(int)ch->in_room->area->zone->number )) == NULL )
    {
    ...
    { 
        in_zone = find_zone(1);
	mob = create_mobile(get_mob_index(MOB_VNUM_ANIMATED_CORPSE,1),in_zone );
	char_to_room( mob, ch->in_room );
...
ch_ret spell_polymorph( int sn, int level, CHAR_DATA *ch, void *vo ) 
{
  ...
  send_to_char("Sorry, spell disabled, for debugging.\n\r",ch);
  return rNONE;
...
*/
  ...
  char_from_room(ch);
  char_to_room(ch, get_room_index(ROOM_VNUM_POLY,1));
...
CHAR_DATA *make_poly_mob(CHAR_DATA *ch, int vnum)
{
  ...
  if ( (pMobIndex = get_mob_index(vnum,(int)ch->in_room->area->zone->number ) ) == NULL)
  {
...
ch_ret spell_affectchar( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	&&   !SPELL_FLAG( skill, SF_ACCUMULATIVE ) )
	   continue;
	    ...
	    case AFF_PARALYSIS: af.type = gsn_stun;		break;
	...
	if ( af.duration == 0 )
	{
		...
		case APPLY_HIT:
		    ...
		    /* Healing mobs to get exp -- nope. --GW */
		    if ( IS_NPC(victim) && victim->fighting &&
			victim != ch)
		    {
		     send_to_char( "You failed.\n\r",ch);
		    ...
		    update_pos( victim );
		    if ( !IS_NPC(ch) &&( (af.modifier > 0 && ch->fighting && ch->fighting->who == victim)
		    ||   (af.modifier > 0 && victim->fighting && victim->fighting->who == ch)) )
		    {
			xp_gain = (long) (ch->fighting->xp * af.modifier*2) / victim->max_hit;
		...
		case APPLY_MOVE:
	        ...
	        case APPLY_ALIGN:
		    if ( !IS_NPC(ch) )
		    victim->alignment = URANGE( -1000, victim->alignment + af.modifier, 1000 );
	            break;
		case APPLY_ROUND_STUN:
		    /* Only allow the person/mob stunned once .. wait till it
   		     * Wears off before we allow it again...--GW
		     */
			
		    /* When a Player named MOBSTER uses web MOB it webs HIM 
			crashing the game -- fixed. --GW */
		    if ( ch == victim )
	            return rSPELL_FAILED;
		    ...
		    victim->stun_rounds = URANGE( 0, victim->stun_rounds + af.modifier, 10 );
		    if ( !victim->fighting )
		...
		case APPLY_EGO:
		  if ( !IS_NPC(victim) )
		    victim->pcdata->ego = URANGE( 0, victim->pcdata->ego + af.modifier,100 );
		    break;
		default:
...
ch_ret spell_obj_inv( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	case SA_CREATE:
	  ...
	  switch( SPELL_POWER(skill) )		/* clone object */
	  {
		...
		if (  obj->cost > ch->level * get_curr_int(ch) * get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_MINOR:
		 if (  obj->cost > ch->level * get_curr_int(ch) / 5 )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_GREATER:
		if (   obj->cost > ch->level * 10 * get_curr_int(ch) *  get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_MAJOR:
		if (  obj->cost > ch->level * 50 * get_curr_int(ch) *  get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
...
ch_ret spell_create_obj( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    zone = find_zone(1);
    if ( (oi=get_obj_index(vnum,(int)ch->in_room->area->zone->number)) ==NULL
    ||   (obj=create_object(oi, lvl,zone)) == NULL )
    {
	failed_casting( skill, ch, NULL, NULL );
	return rNONE;
    }
...
ch_ret spell_create_mob( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( (mi=get_mob_index(vnum,(int)ch->in_room->area->zone->number)) ==NULL
    ||   (mob=create_mobile(mi,in_zone)) == NULL )
    {
	failed_casting( skill, ch, NULL, NULL );
	return rNONE;
    }
...
/*
 * 4 Mentalstate spells
 */

ch_ret spell_dragon_shriek( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    int dam;
   ...
   /* fixed by Callinon (I hope) */
   /* Fixed by Greywolf, leave it alone */
   /* on players, it affects MENTALSTATE, on mobs*/
   /* it affects hit and dam rolls, mentalstate aff's on mobs*/
   /* are lethal */
    if ( !IS_NPC(victim) )
    {    
    af.type      = sn;
    af.duration  = (int)ch->level;
    ...
    {
    modnum = (victim->hitroll * 0.25); /* 25% of there hitroll*/
    af.type      = sn;
    af.duration  = (int)ch->level;
    af.location = APPLY_HITROLL;
    af.modifier = modnum;
    xCLEAR_BITS(af.bitvector);
    affect_to_char( victim, &af );
    modnum = (victim->damroll * 0.25); /* 25% of there damroll*/
    af.type      = sn;
    af.duration  = (int)ch->level;
    ...
    act( AT_MAGIC, "$n's agonizing shriek pierces $N's mind sending him into a mental stupor."
                   , ch, NULL,
	 	  victim, TO_NOTVICT );
    dam = 1;/* remember this?*/
    return damage( ch, victim, dam, sn );
 ...
 */
ch_ret spell_adrenaline_control ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA  *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    act(AT_MAGIC, "$n has given $mself an adrenaline rush!", ch, NULL,
NULL,
	TO_ROOM );
...
ch_ret spell_agitation ( int sn, int level, CHAR_DATA *ch, void *vo )
{
                 CHAR_DATA *victim       = (CHAR_DATA *) vo;
    static const int        dam_each [ ] =
    {
...
ch_ret spell_aura_sight ( int sn, int level, CHAR_DATA *ch, void *vo )
{
...
ch_ret spell_awe ( int sn, int level, CHAR_DATA *ch, void *vo )
  {
    CHAR_DATA *victim = (CHAR_DATA *) vo;
	...
	act(AT_MAGIC, "$N is in AWE of you!", ch, NULL, victim, TO_CHAR
);
	act(AT_MAGIC, "You are in AWE of $n!",ch, NULL, victim, TO_VICT
);
	act(AT_MAGIC, "$N is in AWE of $n!",  ch, NULL, victim, TO_NOTVICT
);
    }
    return rNONE;
}
...
ch_ret spell_ballistic_attack ( int sn, int level, CHAR_DATA *ch, void *vo
)
  {
                 CHAR_DATA *victim       = (CHAR_DATA *) vo;
    static const int        dam_each [ ] =
    ...
    act(AT_MAGIC, "You chuckle as a stone strikes $N.", ch, NULL, victim,
	TO_CHAR );
...
ch_ret spell_cell_adjustment ( int sn, int level, CHAR_DATA *ch, void *vo
)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
...
ch_ret spell_complete_healing ( int sn, int level, CHAR_DATA *ch, void *vo
)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    ...
    ch->mana = 2;
    ch->hit = 2;             /* This thing is just too strong */
    update_pos( ch );         /* but if they really want it I've added a */
    update_pos( victim );     /* penelty to the caster */
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
...
ch_ret spell_control_flames ( int sn, int level, CHAR_DATA *ch, void *vo )
{
                 CHAR_DATA *victim       = (CHAR_DATA *) vo;
    static const int        dam_each [ ] = 
    {
...
ch_ret spell_create_sound ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *vch;
    char       buf1    [ MAX_STRING_LENGTH ];
    char       buf2    [ MAX_STRING_LENGTH ];
...
ch_ret spell_detonate ( int sn, int level, CHAR_DATA *ch, void *vo )
{
                 CHAR_DATA *victim       = (CHAR_DATA *) vo;
    static const int        dam_each [ ] =
    {
...
/* To much of a pain .. forget it --GW */
/**
    if ( number_percent( ) < 2 * level )
	  ...
	  if ( number_bits( 2 ) != 0 )
	      continue;
	   dont kill it if it is a container of it is worn.. loose inv
	     only --GW
	  if ( obj_lose->wear_loc > -1 || obj_lose->item_type == ITEM_CONTAINER || 
	       obj_lose->item_type == ITEM_QUIVER )
	     continue;

	  act(AT_MAGIC, "$p disintegrates!", victim, obj_lose, NULL,TO_CHAR );
	  act(AT_MAGIC, "$p disintegrates!", victim, obj_lose, NULL,TO_ROOM );
...
**/
	...
	act(AT_MAGIC, "You have DISINTEGRATED $N!",         ch, NULL,victim, TO_CHAR );
	act(AT_MAGIC, "You have been DISINTEGRATED by $n!", ch, NULL,victim, TO_VICT );
	act(AT_MAGIC, "$n's spell DISINTEGRATES $N!",       ch, NULL,victim, TO_ROOM );
	damage( ch, victim, 200, TYPE_UNDEFINED );
...
ch_ret spell_enhance_armor (int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	{
	    SET_BIT( obj->extra_flags, ITEM_ANTI_EVIL );
	    act(AT_MAGIC, "$p glows blue.",   ch, obj, NULL, TO_CHAR );
	}
	else if ( IS_EVIL( ch ) )
        {
	    SET_BIT( obj->extra_flags, ITEM_ANTI_GOOD );
	...
	else
	{
	    SET_BIT( obj->extra_flags, ITEM_ANTI_EVIL );
	    SET_BIT( obj->extra_flags, ITEM_ANTI_GOOD );
	    act(AT_MAGIC, "$p glows yellow.", ch, obj, NULL, TO_CHAR );
	}
	...
	act(AT_MAGIC, "$p turns black.", ch, obj, NULL, TO_CHAR );
    }
...
ch_ret spell_intellect_fortress ( int sn, int level, CHAR_DATA *ch, void
*vo )
{
    CHAR_DATA  *gch;
    AFFECT_DATA af;
	...
	act(AT_MAGIC, "A virtual fortress forms around $N.", gch, NULL,
gch, TO_ROOM );
...
ch_ret spell_lend_health ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( hpch == 0 )
    {
	act(AT_MAGIC,"Nice thought, but $N doesn't need healing.", ch,
NULL,
	    victim, TO_CHAR );
	return rNONE;
    }
    ...
    act(AT_MAGIC, "You lend some of your health to $N.", ch, NULL, victim,
TO_CHAR );
    act(AT_MAGIC, "$n lends you some of $s health.",     ch, NULL, victim,
TO_VICT );
...
ch_ret spell_levitation ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    act(AT_MAGIC, "$n's feet rise off the ground.", victim, NULL, NULL,
TO_ROOM );
    return rNONE;
}
...
ch_ret spell_mental_barrier ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA  *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
...
ch_ret spell_thought_shield ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA  *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
...
ch_ret spell_ultrablast ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *vch;
    CHAR_DATA *vch_next;
    int        dam=0;
