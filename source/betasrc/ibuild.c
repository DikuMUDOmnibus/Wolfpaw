...
#ifdef NEW_MAPS
	case 'd':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, room_page_d, buf);
		ch->pcdata->inter_page = ROOM_PAGE_D;
		break;
#endif
	case 'h':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, room_help_page, buf);
		ch->pcdata->inter_page = ROOM_HELP_PAGE;
		break;
	case 'c':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, room_page_c, buf);
		ch->pcdata->inter_page = ROOM_PAGE_C;
		break;
	case 'b':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, room_page_b, buf);
		ch->pcdata->inter_page = ROOM_PAGE_B;
		break;
	case 'a':
	default:
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, room_page_a, buf);
		ch->pcdata->inter_page = ROOM_PAGE_A;
		break;
	}
...
send_control_page_to_char(CHAR_DATA * ch, char page)
{
	...
	case 'h':
		...
		ch->pcdata->inter_page = CONTROL_HELP_PAGE;
		break;
	case 'a':
	default:
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, control_page_a, buf);
		ch->pcdata->inter_page = CONTROL_PAGE_A;
		break;
	}
	offset = get_offset(ch);
...
/*	char            buf[MAX_STRING_LENGTH];*/
	...
	ch->pcdata->inter_editing = NO_PAGE;
/*	sprintf(buf, "\E[%d;%dr", 1, ch->pcdata->pagelen);
	write_to_buffer(ch->desc, buf, strlen(buf));
	write_to_buffer(ch->desc, "50\E[;H\E[2J", strlen("50\E[;H\E[2J"));*/
	write_to_buffer(ch->desc, "\E[2J", 4);
	return;
}
...
send_obj_page_to_char(CHAR_DATA * ch, OBJ_INDEX_DATA * idx, char page)
{
	...
	case 'h':
		...
		ch->pcdata->inter_page = OBJ_HELP_PAGE;
		break;
	case 'e':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, obj_page_e, buf);
		ch->pcdata->inter_page = OBJ_PAGE_E;
		break;
	case 'd':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, obj_page_d, buf);
		ch->pcdata->inter_page = OBJ_PAGE_D;
		break;
	case 'c':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, obj_page_c, buf);
		ch->pcdata->inter_page = OBJ_PAGE_C;
		break;
	case 'b':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, obj_page_b, buf);
		ch->pcdata->inter_page = OBJ_PAGE_B;
		break;
	case 'a':
	default:
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, obj_page_a, buf);
		ch->pcdata->inter_page = OBJ_PAGE_A;
		break;
	}
	offset = get_offset(ch);
...
send_page_to_char(CHAR_DATA * ch, MOB_INDEX_DATA * idx, char page)
{
	...
	case 'h':
		...
		ch->pcdata->inter_page = MOB_HELP_PAGE;
		break;
	case 'f':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, mob_page_f, buf);
		ch->pcdata->inter_page = MOB_PAGE_F;
		break;
	case 'e':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, mob_page_e, buf);
		ch->pcdata->inter_page = MOB_PAGE_E;
		break;
	case 'd':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, mob_page_d, buf);
		ch->pcdata->inter_page = MOB_PAGE_D;
		break;
	case 'c':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, mob_page_c, buf);
		ch->pcdata->inter_page = MOB_PAGE_C;
		break;
	case 'b':
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, mob_page_b, buf);
		ch->pcdata->inter_page = MOB_PAGE_B;
		break;
	case 'a':
	default:
		format_string(buf,ch->pcdata->inter_editing,10);
		sprintf(menu_picture, mob_page_a, buf);
		ch->pcdata->inter_page = MOB_PAGE_A;
		break;
	}
	offset = get_offset(ch);
...
do_redraw_page(CHAR_DATA * ch, char *argument)
{
	...
	if(ch->pcdata->inter_type== MOB_TYPE)
	{
	   idx = get_mob_index(ch->pcdata->inter_editing_vnum,1);
	   if (idx == NULL) {
		   bug("do_redraw_page: Get_mob_index: bad vnum %d.",
ch->pcdata->inter_editing_vnum);
	...
	if(ch->pcdata->inter_type== ROOM_TYPE)
	{
	   ridx = get_room_index(ch->pcdata->inter_editing_vnum,1);
	   if (ridx == NULL) {
...
ch->pcdata->inter_editing_vnum);
	...
	if(ch->pcdata->inter_type== OBJ_TYPE)
	{
	   oidx = get_obj_index(ch->pcdata->inter_editing_vnum,1);
	   if (oidx == NULL) {
...
ch->pcdata->inter_editing_vnum);
	   ...
	   return;
        ...
        if(ch->pcdata->inter_type== CONTROL_TYPE)
        {
           send_control_page_to_char(ch, page);
           return;
        }

        bug("do_redraw_page: Bad ch->pcdata->inter_type: vnum %d.",
ch->pcdata->inter_editing_vnum);
}
...
refresh_page(CHAR_DATA * ch)
{
	...
	case MOB_TYPE:
		...
		idx = (MOB_INDEX_DATA *)
get_mob_index(ch->pcdata->inter_editing_vnum,1);
		if (idx == NULL) {
...
ch->pcdata->inter_editing_vnum);
	...
	case ROOM_TYPE:
		...
		idx = (ROOM_INDEX_DATA *)
get_room_index(ch->pcdata->inter_editing_vnum,1);
		if (idx == NULL) {
...
ch->pcdata->inter_editing_vnum);
	...
	case OBJ_TYPE:
		...
		idx = (OBJ_INDEX_DATA *)
get_obj_index(ch->pcdata->inter_editing_vnum,1);
		if (idx == NULL) {
...
fill_in_control_page(CHAR_DATA * ch, char page)
{
	...
	if ((ch->pcdata->inter_substate < 0) || (ch->pcdata->inter_substate >
5))
{
	...
	sprintf(buf, "\E[%d;%dr", offset, ch->pcdata->pagelen);
	write_to_buffer(ch->desc, buf, strlen(buf));

	/* goes to pglen,1 */
	sprintf(buf, "\E[%d;1H", (ch->pcdata->pagelen));
	write_to_buffer(ch->desc, buf, strlen(buf));
...
fill_in_room_page(CHAR_DATA * ch, ROOM_INDEX_DATA * idx, char page)
{
	...
	if ((ch->pcdata->inter_substate < 0) || (ch->pcdata->inter_substate > 5))
{
		send_to_char("Bad interface substate. Can't fill in page.\n\r", ch);
		return;
	}
	...
	case 'c':
		...
		room_page_c_data[0].data = (ch->pcdata->inter_substate ==
SUB_NORTH) ? check : space;
		room_page_c_data[1].data = (ch->pcdata->inter_substate == SUB_UP)
? check : space;
		room_page_c_data[2].data = (ch->pcdata->inter_substate ==
SUB_EAST) ? check : space;
		room_page_c_data[3].data = (ch->pcdata->inter_substate ==
SUB_WEST) ? check : space;
		room_page_c_data[4].data = (ch->pcdata->inter_substate ==
SUB_SOUTH) ? check : space;
		room_page_c_data[5].data = (ch->pcdata->inter_substate ==
SUB_DOWN) ? check : space;
		xit = get_exit(idx, ch->pcdata->inter_substate);
		if (!xit) {
			...
			if ((pObjIndex = get_obj_index(xit->key,1)) ==
NULL) {
...
/*		mob_page_b_data[17].data = IS_SET(idx->affected_by, AFF_BLIND) ?
check : space;
