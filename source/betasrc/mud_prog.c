...
void init_supermob()
{
   ...
   supermob = create_mobile(get_mob_index( 3,1 ),in_zone);
   office = get_room_index ( 3,1 );
  ...
  *pchck = '\0';
  ...
  if ( !str_cmp(chck, "economy") )
  {
    ...
    if ( !idx )
    {
      ...
      room = get_room_index(idx,mob->in_room->area->zone->number);
    if ( !room )
    {
  ...
  if ( !str_cmp(chck, "timeskilled") )
  {
    ...
    else if ( !(pMob =get_mob_index(atoi(cvar),mob->in_room->area->zone->number)) )
    {
  ...
  if ( chkchar )
  {
    ...
    if ( !str_cmp(chck, "isclanwf") )
    {
     if ( IS_NPC(chkchar) )
     return FALSE;
   
     if ( chkchar->pcdata && chkchar->pcdata->clan )
     {
      if ( !str_cmp(chkchar->pcdata->clan->name, "-<WF>-" ) );
        return TRUE;
     }
     return FALSE;
    }
...
/*    if ( !str_cmp(chck, "wasinroom") )
    ...
    if ( !str_cmp(chck, "advclass") )
    {
      if ( IS_NPC(chkchar) || !IS_ADVANCED(chkchar) )
	return -1;

      return mprog_seval((char *)class_table[chkchar->advclass]->who_name,
	   opr, rval, mob);
    }
                      ...
                      bool ignore, bool ignore_ors )
{
  ...
  *point = '\0';
  ...
  /* Trying to fix Oprogs! --GW */
  if ( obj )
    interpret( supermob, buf );
  else  
    interpret( mob, buf );  
...
void release_supermob( )
{
  if (supermob->in_room)
    char_from_room( supermob );
  char_to_room( supermob, get_room_index( 3,1 ) );
}
			 ...
			 void *vo, int type)
{
 ...
 for ( mprg = obj->pIndexData->mudprogs; mprg != NULL; mprg = mprg->next )
   if ( ( mprg != NULL && type && mprg->type == type )
       && ( number_percent( ) <= atoi( mprg->arglist ) ) )
     {
       
       if ( mprg->comlist && mob && actor && obj )
       {
       executed = TRUE;
       mprog_driver( mprg->comlist, mob, actor, obj, vo, FALSE );
       }
       if ( type && type != GREET_PROG )
	 break;
     }
	...
	if ( ( list[0] == 'p' ) && ( list[1] == ' ' ) )
	  {
		...
		{
		  set_supermob( iobj );
		  mprog_driver( mprg->comlist, supermob, actor, obj, vo, FALSE );
		  release_supermob() ;
		  break;
		}
	      else
