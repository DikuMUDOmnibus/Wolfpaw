...
void interpret( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC(ch) && (fun=ch->pcdata->last_cmd) == NULL )
	{
	 ...
	 * Aliens --GW
	 */
	if ( !IS_NPC(ch) && IS_SET(ch->pcdata->flagstwo, MOREPC_ABDUCTING) )
	{
	    send_to_char( "You're to scared to move!\n\r", ch );
	    return;
	}
...
if ( !IS_NPC(ch) && IS_SET(ch->pcdata->flags, PCFLAG_CHEATER) )
{	
...
||  !str_prefix( command, "gossip" )
||  !str_prefix( command, "say" )
||  !str_prefix( command, "tell" )
||  !str_prefix( command, "flee" )
||  !str_prefix( command, "quit" )
||  !str_prefix( command, "who" )
||  !str_prefix( command, "look" )
||  !str_prefix( command, "recall" )
||  !str_prefix( command, "use" ) )
{
     send_to_char("Ok, but Your a CHEATER, TALK TO A GOD!!\n\r",ch);
     sprintf(lag_buf,"%s 50",ch->name);
     do_addlag(ch,lag_buf);
     send_to_char("Oops, you seem to be lagging *innocent*\n\e",ch);
}
else
{
     send_to_char("NO!!! Your a CHEATER, TALK TO A GOD!!\n\r",ch);
     return;
}     
...
/*	if ( ch->desc && ch->desc->original )
    ...
    {
  	sprintf( logname, "%s", ch->name);
	write_to_gallery(ch, logname );
	write_to_gallery(ch, "> " );
	write_to_gallery(ch, logline );
	write_to_gallery(ch, "\n\r" );
    }
	...
	&&   !check_social( ch, command, argument ) )
	{
	  ...
	  if ( rannum == 1 )
	    ...
	    send_to_char( "Are you on GLUE?!?!?\n\r", ch );
	  else if ( rannum == 3 )
	    send_to_char( "How the F*CK did you get to this level!?!\n\r", ch );
	  else if ( rannum == 4 )
	    send_to_char( "S'cuse me??\n\r", ch );
	  else if ( rannum == 5 )
	    send_to_char( "You know, I do have the number to a good *Typotheripist*\n\r", ch );
	  else if ( rannum == 6 )
	    send_to_char( "Connection closed by foriegn host.\n\r", ch );
	  else if ( rannum == 7 )
	    send_to_char( "Your totally frozen!\n\r", ch );
	  else if ( rannum == 8 )
	    send_to_char( "Sorry, that makes no sense.\n\r", ch );
/*    Justice: Adding new typo messages */
          else if ( rannum == 9 )
            send_to_char( "What've you been smoking and why haven't you been sharing?\n\r", ch );
          else if ( rannum == 10 )
            send_to_char( "What's that in English?\n\r", ch );
	}
	return;
    }
...
*/
	/* Ability to forbid someone to use a command --GW */
	    if ( !IS_NPC(ch) && ch->pcdata->bestowments[0] != '\0' &&
		 is_name_with_minus( cmd->name,ch->pcdata->bestowments ) )
	    {
		send_to_char("You are not allowed to use this command.\n\r",ch);
		return;
	    }	     

	/* Hyp Retired Command Restriction to level 51.. --GW */
	if ( IS_RETIRED( ch ) && IS_IMP(ch) && cmd->level > 51 )
	{
	  send_to_char("&wYour Retired.. please speak to an Active Hyperion.\n\r",ch);
	  return;
	}
	...
	/*
  	 * Last 10 commands logged in pfile for gods to look at
	 * at Wyverns Request --GW
         */
	if( !IS_NPC(ch) )
	{
	 for ( comnum = 0; comnum < MAX_COM; comnum++ )
	 {
	  if ( !ch->pcdata->lastcom[comnum] )
	    break;

           ch->pcdata->comtrack[comnum+1] = STRALLOC(ch->pcdata->lastcom[comnum]);
	 }

            ch->pcdata->comtrack[0] = STRALLOC(logline);
	
	 for( comnum2 = 0; comnum2 < MAX_COM; comnum2++ )
	 {
	 if (ch->pcdata->comtrack[comnum2] == NULL )
	   break;

            ch->pcdata->lastcom[comnum2] = STRALLOC(ch->pcdata->comtrack[comnum2]);
	 }
        }	 
    /*
...
SOCIALTYPE *find_social( CHAR_DATA *ch, char *command, bool personal )
{
    SOCIALTYPE *social;
    int hash;
    ...
    if ( personal )
    {
      if ((social=personal_social_check(ch,command))!=NULL)
	return social;
    }
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    CMAP_DATA *map;
    int colornum=0;
    bool personal_found=FALSE;

    if ( (social=find_social(ch,command,FALSE)) == NULL )
    {
      if ( (social=find_social(ch,command,TRUE)) == NULL )
	return FALSE;
      else
        personal_found = TRUE;
    }
    ...
    victim = NULL;
  if ( !colormap_check(ch,CMAP_SOCIALS) )
  {
    if ( arg[0] == '\0' )
    {
	act( AT_SOCIAL, social->others_no_arg, ch, NULL, NULL, TO_ROOM );
	act( AT_SOCIAL, social->char_no_arg,   ch, NULL, NULL, TO_CHAR );
    }
    ...
    if ( !IS_NPC(victim) )
    {
    send_to_char("From Far Away...",ch);
    act( AT_SOCIAL, social->char_found, ch, argument, victim,TO_CHAR );
    send_to_char("From Far Away...",victim);
    act( AT_SOCIAL, social->vict_found, ch, argument, victim,TO_VICT );
    }
    }
    else if ( victim == ch )
    {
    ...
    if ( IS_NPC(victim ) && ( victim->fighting && victim->fighting->who) )
    {
	if ( personal_found )
	   free_socialtype_data(social);
    return FALSE;
  ...
  {
    map=find_char_map_by_type(ch,CMAP_SOCIALS);
    colornum=get_real_act_color_num(map->mapped_color);

    if ( arg[0] == '\0' )
    {
	act( colornum, social->others_no_arg, ch, NULL, NULL, TO_ROOM );
	act( colornum, social->char_no_arg,   ch, NULL, NULL, TO_CHAR );
    }
    else if ( ( victim = get_char_room( ch, arg ) ) == NULL )    
    {
    if ( ( victim = get_char_world( ch, arg ) ) == NULL )
    ...
    {
    send_to_char("From Far Away...",ch);
    act( colornum, social->char_found, ch, argument, victim,TO_CHAR );
    send_to_char("From Far Away...",victim);
    act( colornum, social->vict_found, ch, argument, victim,TO_VICT );
    }
    }
    else if ( victim == ch )
    {
	act( colornum, social->others_auto,   ch, NULL, victim, TO_ROOM    );
	act( colornum, social->char_auto,     ch, NULL, victim, TO_CHAR    );
    }
    else
    {
	act( colornum, social->others_found,  ch, NULL, victim, TO_NOTVICT );
	act( colornum, social->char_found,    ch, NULL, victim, TO_CHAR    );
	...
	act( colornum, social->vict_found,    ch, NULL, victim, TO_VICT    );
        ...
        &&   !IS_SET( victim->pIndexData->progtypes, ACT_PROG ) )
	{
		...
		if ( IS_NEUTRAL(ch) )
		{
		    act( colornum, "$n slaps $N.",  victim, NULL, ch, TO_NOTVICT);
		    act( colornum, "You slap $N.",  victim, NULL, ch, TO_CHAR);
		    act( colornum, "$n slaps you.", victim, NULL, ch, TO_VICT);
		}
		else
		{
		    act( colornum, "$n acts like $N doesn't even exist.",  victim,NULL, ch, TO_NOTVICT );
		    act( colornum, "You just ignore $N.",  victim, NULL, ch,TO_CHAR );
		    act( colornum, "$n appears to be ignoring you.", victim, NULL,ch, TO_VICT    );
		}
		break;

	    case 1: case 2: case 3: case 4:
	    case 5: case 6: case 7: case 8:
		act( colornum, social->others_found,
		    victim, NULL, ch, TO_NOTVICT );
		act( colornum, social->char_found,
		    victim, NULL, ch, TO_CHAR    );
		act( colornum, social->vict_found,
		    victim, NULL, ch, TO_VICT    );
		break;
...
char *one_argument( char *argument, char *arg_first )
{
    ...
    /* Appearently the MUD is being crashed with buffer exploits simular to those used in shells to bust
       root.. well just stop that dead..GW */
    while ( ((*argument != '\0') && (++cnt < MIL) )|| ++count >= 255 )
    {
	if ( *argument == cEnd )
	{
	    argument++;
    ...
    /* hopefully put a noose on the crap bug --GW */
    if ( !isprint(*argument) )
    {
	argument++;
	break;
...
char *one_argument2( char *argument, char *arg_first )
{
    ...
    while ( ((*argument != '\0') && (++cnt < MSL)) || ++count >= 255 )
    {
