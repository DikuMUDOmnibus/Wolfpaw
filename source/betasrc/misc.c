...
void do_fill( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("Use the PUT Command.\n\r",ch );
	return;
...
void do_drink( CHAR_DATA *ch, char *argument )
{
	...
	maxnum = ch->max_hit * 2;
        ch->hit  += liq_table[liquid].liq_affect[3];
        ch->mana += liq_table[liquid].liq_affect[4];
        ch->move += liq_table[liquid].liq_affect[5];
	if ( ch->hit > maxnum )
	  ch->hit = maxnum;
...
void do_eat( CHAR_DATA *ch, char *argument )
{
	...
	return;
     ...
     {
	send_to_char("Um... No.. doing that right now would crash the mud!\n\r",ch);
	return;
     }
...
*/
        if ( IS_SET( ch->in_room->room_flags, ROOM_NO_MAGIC ) )
        {
            ...
            act(AT_MAGIC,"$n tries to quaff $p, but it tastes like water!",ch,obj,NULL,TO_ROOM);
            act(AT_MAGIC,"You try to quaff $p, but it tastes like water!",ch,obj,NULL,TO_CHAR);
            return;
        }
...
void pullorpush( CHAR_DATA *ch, OBJ_DATA *obj, bool pull )
{
	...
	if ( ( room = get_room_index( obj->value[1],zone ) ) == NULL )
	{
    ...
    {
	room = get_room_index( obj->value[1],zone );
	if ( !room )
	  room = obj->in_room;
	if ( !room )
	{
	...
	if ( !pexit )
	{
	    ...
	    to_room = get_room_index( obj->value[2],zone );
	    if ( !to_room )
	    {
