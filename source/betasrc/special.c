...
DECLARE_SPEC_FUN(       spec_clan_salesman	);
DECLARE_SPEC_FUN(       spec_glory_salesman	);
...
SPEC_FUN *spec_lookup( const char *name )
{
    ...
    if ( !str_cmp( name, "spec_clan_salesman"     ) ) return spec_clan_salesman;
    if ( !str_cmp( name, "spec_glory_salesman"    ) ) return spec_glory_salesman;
    return 0;
}
...
char *lookup_spec( SPEC_FUN *special )
{
    ...
    if ( special == spec_clan_salesman  )       return "spec_clan_salesman";
    if ( special == spec_glory_salesman )       return "spec_glory_salesman";
    return "";
}
...
{
    record_call("<spec_questmaster>");
    return TRUE;
}

bool spec_clan_salesman( CHAR_DATA *ch )
{
    record_call("<spec_clan_salesman>");
    return TRUE;
}

bool spec_glory_salesman( CHAR_DATA *ch )
{
    record_call("<spec_glory_salesman>");
    return TRUE;
}

/* if a spell casting mob is hating
someone... try and summon them */
...
{
   record_call("<spec_breath_lightning>");
...
bool spec_cast_adept( CHAR_DATA *ch )
{
   ...
   record_call("<spec_cast_adept>");
	...
	if ( victim && victim != ch && can_see( ch, victim ) && number_bits( 1 ) == 0 )
	    break;
    }
    ...
    {
    case 0:
    act( AT_MAGIC, "$n utters the word 'edaj'.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "armor" ), ch->level, ch, victim );
	return TRUE;

    case 1:
    act( AT_MAGIC, "$n utters the word 'namsnik'.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "bless" ), ch->level, ch, victim );
	return TRUE;

    case 2:
    act( AT_MAGIC, "$n utters the word 'retsasid'.", ch, NULL, NULL,TO_ROOM );
	spell_cure_blindness( skill_lookup( "cure blindness" ),
	    ch->level, ch, victim );
	return TRUE;

    case 3:
    act( AT_MAGIC, "$n utters the word 'artnes'.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "cure light" ),
	    ch->level, ch, victim );
	return TRUE;

    case 4:
    act( AT_MAGIC, "$n utters the word 'snrevyw'.", ch, NULL, NULL,TO_ROOM );
	spell_cure_poison( skill_lookup( "cure poison" ),
	    ch->level, ch, victim );
	return TRUE;

    case 5:
    act( AT_MAGIC, "$n utters the word 'ynattirb'.", ch, NULL, NULL,TO_ROOM );
	spell_smaug( skill_lookup( "refresh" ), ch->level, ch, victim );
	return TRUE;

    case 6:
    act( AT_MAGIC, "$n utters the word 'flowyerg'.", ch, NULL, NULL, TO_ROOM );
 	spell_remove_curse( skill_lookup( "remove curse" ), ch->level, ch, victim);
	return TRUE;

    case 7:
    act( AT_MAGIC, "$n utters the word 'nonillac'.", ch, NULL, NULL, TO_ROOM );
...
/* Commented out by Narn, Nov 28/95
	...
	default: min_level = 35; spell = "comet";          break;
	}
...
bool spec_executioner( CHAR_DATA *ch )
{
   ...
   record_call("<spec_executioner>");
    ...
    cityguard = get_mob_index( MOB_VNUM_CITYGUARD,1 );
    ...
    char_to_room( create_mobile( cityguard,in_zone ), ch->in_room );
    char_to_room( create_mobile( cityguard,in_zone ), ch->in_room );
    return TRUE;
}
...
bool spec_mayor( CHAR_DATA *ch ) /* What is the point of this anyway - Cal */
{
