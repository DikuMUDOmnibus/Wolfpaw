...
void do_noteroom( CHAR_DATA *ch, char *argument )
{
    ...
    if ( board->type == BOARD_PK && 
         str_cmp(arg, "list" ) )
    {
      send_to_char("You Cannot do that to this board.\n\r",ch);
      return;
    }
    ...
    if ( board->type == BOARD_PK && 
         !str_cmp(arg, "list" ) )
    {
      display_pkill_chart(ch);
      return;
    }

    if (board->type != BOARD_NOTE && board->type != BOARD_PK)
    {
	...
	*/
	...
	if (to && strchr(to, '@')!=NULL)
	{
	  ...
	  if ( !text || !*text )
	  {
       ...
       if ( (to && strchr(to, '@')!=NULL) && board->type != 1)
	  {
	    send_to_char( "There is no mail board here.\n\r", ch );
	    return;
	  }
	  imc = TRUE;
	}
...
#define KEY( literal, field, value )					\
  ...
  /* Level Checks --GW */
  if ( board->min_read_level > LEVEL_HYPERION )
   board->min_read_level = LEVEL_HYPERION;
  if ( board->min_post_level > LEVEL_HYPERION )
   board->min_post_level = LEVEL_HYPERION;
  if ( board->min_remove_level > LEVEL_HYPERION )
   board->min_remove_level = LEVEL_HYPERION;
...
void do_bset( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if ( value > get_trust(ch) || board->min_post_level > get_trust(ch))
	{
	...
	if ( !argument || argument[0] == '\0' )
	{
        ...
        if ( board->min_read_level > get_trust(ch) )
	{
	    send_to_char("I Think Not...\n\r",ch);
	    return;
	}
	...
	if ( !argument || argument[0] == '\0' )
	{
        ...
        if ( board->min_post_level > get_trust(ch) )
	{
	    send_to_char("I Think Not...\n\r",ch);
	    return;
	}
   ...
   if ( !str_cmp( arg2, "extra_removers" ) )
    {
        ...
        if ( board->min_remove_level > get_trust(ch) )
	{
	    send_to_char("I Think Not...\n\r",ch);
	    return;
	}
	...
	if ( value < 0 || value > MAX_LEVEL )
	{
	  ...
	  return;
        ...
        if ( value > get_trust(ch) || board->min_remove_level > get_trust(ch))
	{
	    send_to_char("I Think Not...\n\r",ch);
	    return;
	}
