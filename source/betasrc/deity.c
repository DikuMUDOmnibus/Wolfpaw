...
void fread_deity( DEITY_DATA *deity, FILE *fp )
{
	...
	case 'R':
	    ...
	    KEY( "recall",	deity->recall,		fread_number( fp ) );
	    break;
...
void do_setdeity( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "scorpse savatar sdeityobj recall\n\r", ch );
	return;
    }
      ...
      while ( argument[0] != '\0' )      
        {
           ...
           if ( !str_cmp ( arg3, "none" ) )
           ...
           else
            {
             ...
             {
              xTOGGLE_BIT( deity->affected, 1 << value );
              fMatch = TRUE;
             }
            }
        }
...
/*
	...
	ch->pcdata->favor = -1000;
	send_to_char( "A terrible curse afflicts you as you forsake a deity!\n\r", ch );
	if ( IS_AFFECTED( ch->affected_by, ch->pcdata->deity->affected ) )
	{	
	   xREMOVE_BIT( ch->affected_by, ch->pcdata->deity->affected );
	}
	if ( IS_SET( ch->resistant, ch->pcdata->deity->element ) )
	{
	   REMOVE_BIT( ch->resistant, ch->pcdata->deity->element );
...
for(in_zone = first_zone; in_zone; in_zone = in_zone->next )
{
   ...
   if ( !str_cmp( arg, "avatar" ) )
   {
	...
	in_zone = find_zone(ch->in_room->area->zone->number); 
        pMobIndex = get_mob_index( ch->pcdata->deity->avatar,1 );
   ...
   if ( !str_cmp( arg, "object" ) )
   {
	...
	pObjIndex = get_obj_index( ch->pcdata->deity->deityobj,1 );
		in_zone = find_zone(1);
	obj = create_object( pObjIndex, ch->level,in_zone );
   ...
   if ( !str_cmp( arg, "recall" ) )
   {
	...
	if ( ch->pcdata->favor < ch->pcdata->deity->recall )
	{
	   send_to_char( "You do not have enough favor for such a supplication.\n\r", ch );
 	   return;
	}

	if ( ( ch->in_room == get_room_index(6,1) )
	...
	if ( !IS_NPC( ch ) && !location && ch->level >= 5
    	&&   IS_SET( ch->pcdata->flags, PCFLAG_DEADLY ) )
       	  location = get_room_index( 3009,1 );
	...
	if ( !location )
    	...
    	do_look( ch, "auto" );
    	ch->pcdata->favor -= ch->pcdata->deity->recall; 
    	return;
    }
