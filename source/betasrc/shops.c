...
CHAR_DATA *find_keeper( CHAR_DATA *ch )
{
    ...
    if ( !IS_NPC(ch) && IS_SET(ch->act, PLR_KILLER) && 
         !IS_SET( keeper->act, ACT_SELL_TO_ALL ) )
    {
...
int get_repaircost( CHAR_DATA *keeper, OBJ_DATA *obj )
{
	...
	/* Acid Repair Cost --GW */
	if ( IS_SET(keeper->acttwo, ACT2_ACID_REPAIR) && obj->item_type == ITEM_ARMOR &&
	     obj->value[4] > 0 )
	{
  	  cost += (obj->value[4] * 3000000);
	  do_say(keeper,"This Item is Acided.. it will cost you..");
	}
    }
...
void do_buy( CHAR_DATA *ch, char *argument )
{
   ...
   record_call("<do_buy>");
    ...
    for ( mob = ch->in_room->first_person; mob != NULL; mob = mob->next_in_room )
    {
        ...
        if (mob->spec_fun == spec_lookup( "spec_clan_salesman" ))
	break;
    }
	...
	pRoomIndexNext = get_room_index( ch->in_room->vnum +1,ch->in_room->area->zone->number );
	if ( !pRoomIndexNext )
	{
	...
	if ( ch->silver < 10 * pet->level * pet->level )
	{
	...
	pet		= create_mobile( pet->pIndexData,in_zone );
	...
	bool exchange = FALSE;  /* Exchanged Currency? */
	...
	if ( ch->silver < cost )
	{
	  if ( shop_auto_exchange(ch,keeper,cost)==FALSE)
	    return;
	  else 
	   exchange = TRUE;
	}
	...
	if ( IS_OBJ_STAT( obj, ITEM_INVENTORY ) )
	{
	    ...
	    buy_obj = create_object( obj->pIndexData, obj->level,in_zone );
	    if ( !buy_obj )
	    {
		do_say(keeper,"Sorry, I cannot sell you that many!");
		return;
	    }
	    ...
	    if ( noi > 1 )
	    {
		bag = create_object( get_obj_index(OBJ_VNUM_SHOPPING_BAG,1), 1,first_zone );
		/* perfect size bag ;) */
...
void do_list( CHAR_DATA *ch, char *argument )
{
   CHAR_DATA *mob;
	...
	for ( obj = keeper->first_carrying; obj; obj = obj->next_content )
	{
		...
		{
		    found = TRUE;
		    send_to_char( "[Lv Price Currency] Item\n\r", ch );
		}
		ch_printf( ch, "[%2d %5d Silver] %s.\n\r",
                 ...
                 char *arg, int maxgold, char *fixstr, char*fixstr2 )
{
   ...
   {
      sprintf( buf,
       "$N tells you, 'It will cost %d piece%s of copper to %s %s...'", cost,
   ...
   else
   {
      ...
      if ( IS_SET(keeper->acttwo, ACT2_ACID_REPAIR) && obj->item_type == ITEM_ARMOR &&
	   obj->value[4] > 0 )
      {
         sprintf( buf, "$N repairs the Acid Damage..");
         act( AT_RED, buf, ch, NULL,keeper,TO_ROOM);
      }

      sprintf( buf, "$N charges you %d copper piece%s to %s $p.",
      ...
      if ( keeper->gold > maxgold )
      {
          ...
          act( AT_ACTION, "$n puts some copper into a large safe.", keeper, 
		NULL, NULL, TO_ROOM );
      }
...
void appraise_all( CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr )
{
        ...
        ||   obj->item_type == ITEM_STAFF ) )
	{
            ...
            {
            sprintf( buf,
            "$N tells you, 'It will cost %d piece%s of copper to %s %s'",
    ...
    if ( total > 0 )
    {
       send_to_char ("\n\r", ch);
       sprintf( buf,
          "$N tells you, 'It will cost %d piece%s of copper in total.'",
          total, cost == 1 ? "" : "s" );
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
       strcpy( pbuf,
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
       "$N tells you, 'It will cost %d piece%s of copper to %s that...'", cost,
       cost == 1 ? "" : "s", fixstr );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    if ( cost > ch->gold )
      act( AT_TELL, "$N tells you, 'Which I see you can't afford.'", ch,
...
void do_makeshop( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (mob = get_mob_index(vnum,ch->in_room->area->zone->number)) ==NULL )
    {
...
void do_shopset( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( (mob2 = get_mob_index(vnum,ch->in_room->area->zone->number))== NULL )
	{
	    send_to_char( "Mobile not found.\n\r", ch );
	    return;
	}
...
void do_repairset( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg1 );
    argument = one_argument( argument, arg2 );
    record_call("<do_repairset>");

    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	send_to_char( "Usage: repairset <mob vnum> <field> value\n\r", ch );
	send_to_char( "\n\rField being one of:\n\r", ch );
	send_to_char( "  fix0 fix1 fix2 fix3 profit type open close keeper\n\r", ch );
	return;
    }
