...
void draw_map (CD * ch, RID * rm, int flag, int mode) {
	...
	for (x = 0; x < 78; x++) {
	    ...
	    tmp_rm = get_room_index( map_index -> map_of_vnums[y][x],zone) ;
	    if (tmp_rm == NULL) {
		text_map[i] = ' ';
		i++;
		continue;
...
void do_mapout (CD * ch, char *argument) {
	...
	map_obj_index = get_obj_index (5013,1);
	if(!map_obj_index)
	{
	    map_obj = create_object (map_obj_index , 0,first_zone);
...
add_new_room_to_map(CD *ch,MID *map,int row,int col,int proto_room,char code) {
     ...
     */
    rm = get_room_index (proto_room,1);
    if (!rm)
	rm = ch -> in_room;
	...
	if (get_room_index (i,1) == NULL) {
	    location = make_room (i,1);
	    if (!location) {
...
void map_to_rooms (CD * ch, MID * m_index) {
    ...
    */
	...
	for (x = 0; x < 78; x++) {/* cols (78, i think) */
	    ...
	    newrm = get_room_index (map[y][x].vnum,1);
		...
		if ((tvnum = map[y - 1][x].vnum) != 0) {
		    ...
		    if (exit_type > -1) {
			xit = make_exit( newrm, get_room_index( tvnum,1 ),DIR_NORTH );
		...
		if ((tvnum = map[y][x + 1].vnum) != 0) {
		    ...
		    if (exit_type > -1) {
			xit = make_exit( newrm, get_room_index( tvnum,1 ),DIR_EAST );
			xit->keyword  		= STRALLOC( "" );
			xit->description	= STRALLOC( "" );
			xit->key 		= -1;
			xit->exit_info 		= exit_type;
		...
		if ((tvnum = map[y + 1][x].vnum) != 0) {
		    exit_type = exit_lookup (map[y][x].proto_vnum,
			    map[y + 1][x].proto_vnum);
		    if (exit_type > -1) {
			xit = make_exit( newrm, get_room_index( tvnum,1 ),DIR_SOUTH );
			xit->keyword  		= STRALLOC( "" );
			xit->description	= STRALLOC( "" );
			xit->key 		= -1;
			xit->exit_info 		= exit_type;
		...
		if ((tvnum = map[y][x - 1].vnum) != 0) {
		    exit_type = exit_lookup (map[y][x].proto_vnum,
			    map[y][x - 1].proto_vnum);
		    if (exit_type > -1) {
			xit = make_exit( newrm, get_room_index( tvnum,1 ),DIR_WEST );
			xit->keyword  		= STRALLOC( "" );
			xit->description	= STRALLOC( "" );
			xit->key 		= -1;
			xit->exit_info 		= exit_type;
...
int     exit_lookup (int vnum1, int vnum2) {
    ...
    rm1 = get_room_index (vnum1,1);
    rm2 = get_room_index (vnum2,1);
